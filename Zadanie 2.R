#Задание 2
#Куприянов Алексей 124 группа ПАЭ - создайте модель множественной линейной регрессии потоков углекислого газа за весенний период 
...
setwd("D:/Group_124/Kupriyanov/MathMod2")
getwd()
#cчитаем данные из файла, пропускаем первую строку, заменяем текстовые 'NA', пустые и сгенерированные пороговые значения на NA, игнорируем строки с "[" 
eddypro = read_csv("eddypro.csv", skip = 1, na=c("","NA","-9999","-9999.0"), comment=c("["))
eddypro = eddypro[-1, ]
eddypro = select(eddypro, -(roll))
#преобразуем значения в факторные
...
#Выберем данные за весенний период
